//
//  DoraemonController.m
//  MyCode
//
//  Created by 中付支付 on 2017/7/7.
//  Copyright © 2017年 zzf. All rights reserved.
//

#import "DoraemonController.h"
#import "UIImage+scale.h"
#import "UIWebView+ToFile.h"

#define Width SCREEN_WIDTH

@interface DoraemonController ()

@end

@implementation DoraemonController
...
- (void)viewDidLoad {
    ...
    CGFloat arcCenterX=SCREEN_WIDTH/2;
    CGFloat arcCenterY= 120+headerCornerRadius;
    
    
    //画头部
    CAShapeLayer *headLayer=[[CAShapeLayer alloc] init];
    UIBezierPath *headerPath=[UIBezierPath bezierPathWithRoundedRect:CGRectMake(Width/2-headerCornerRadius, 120, headerCornerRadius*2, headerCornerRadius*2) cornerRadius:headerCornerRadius];
    [self setLayer:headLayer path:headerPath];
    
    //脸
    CAShapeLayer *faceLayer=[CAShapeLayer layer];
    UIBezierPath *facePath=[UIBezierPath bezierPathWithArcCenter:CGPointMake(Width/2, 120+headerCornerRadius+10) radius:70 startAngle:M_PI endAngle:3*M_PI clockwise:YES];
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(Width/2+90, 120, 70, 30)];
